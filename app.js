...
function unitedStatesToggle(event) {
    ...
    $('#states-js').removeClass('hide')
    $('#provinces-js').addClass('hide')
...
function canadaToggle(event) {
    event.preventDefault();
    $('.select-container').removeClass('hide')
    $('#provinces-js').removeClass('hide')
    $('#states-js').addClass('hide')

}

function renderHtml() {
    let cost = info.findCost()
    let costHTML = "<h3>The estimated cost is: " + "$" + cost + "</h3>"
    $('.main').removeClass('blur')
...
function handleXML(err, xml, cb) {
    if(err) {
        console.log(err)
        alert('Sorry there was an error finding your vehicle, we have set a default mileage please try again')
        $('.main').removeClass('blur')
        $('.loading-msg-calc').hide()
        $('.loading').hide()
        $('.cost-container').empty()
...
$(document).ready(function() {
    ...
    $('.input-form').on('submit', calculateCost)
    //$('.btn-calc').on('submit', calculateCost);
